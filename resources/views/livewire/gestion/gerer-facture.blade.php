<div>
    ...
    <div>
        <div class="container   ">
            <div class="row ">
                <div class="col-lg-4">
                    ...
                    <h3  id="titre-prof"> Facture</h3>
                    <p id="text-prof">Gerer  / Facture</p>   
                </div>
        ...
        <div class="container   p-2 rounded-2" style="font-size: 0.75rem">
            <form wire:submit.prevent="deleteSelected">
                <div class="bg-white rounded-3 shadow-sm">
                    ...
                    <table class="  table  card-body  rounded-3 shadow-sm border-0  " style="border:0;overflow:hidden" >
                <div class="row  p-2">
                    <div class="col-lg-2">
                        <h5 id="titre-prof" class="m-1 fw-bold">Liste de facture</h5>
                    </div>
                ...
                <thead class="">
                    ...
                    <th class="fw-bold p-2 border-0" >N° du facture</th>
                    <th class="fw-bold p-2 border-0" >N° du devis</th>
                    {{-- <th class="fw-bold p-2 border-0" >Nature du projet</th> --}}
                    <th class="fw-bold p-2 border-0" >date d'édition</th>
                    <th class="fw-bold p-2 border-0" >Date d'envoie</th>
                    <th class="fw-bold p-2 border-0" >Montant (en AR)</th>
            ...
            <tbody class="p-2 border-0">
                ...
                @foreach ($factures as $dat )                           
                <tr>
                    <td class="bg-white p-2 border-0" >
                        <input type="checkbox" wire:model="checkData" value="{{$dat->id}}"  class="border-0">
                    </td>
                    <td  class="text-muted bg-white p-1 border-0" id="tdanim2">{{$dat->id}}</td>
                    <td  class="text-muted bg-white p-1 border-0" id="tdanim2">{{$dat->id_devis}}</td>
                    {{-- <td  class="text-muted bg-white p-1 border-0" id="tdanim1">{{$dat->nature_projet}}</td> --}}
                    <td  class="text-muted bg-white p-1 border-0" id="tdanim2">
                        {{
                    ...
                    <td  class="text-muted bg-white p-1 border-0" id="tdanim1">  
                        {{
                        $rr = date(" d M Y ",strtotime($dat->date_envoi));
                    }}</td>
                    <td  class="text-muted bg-white p-1 border-0" id="tdanim1">{{$dat->montant}}</td>
               ...
               <hr>
                <form wire:submit.prevent="create">
                    @csrf
                          ...
                          <input type="text" id="nom" wire:model='titre'
                          ...
                          <input type="text" wire:model="description"
                            class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm"
                             placeholder="Déscription du projet" required>        
      
                
                          <label for="contact" class="text-muted m-2 fw-bold">{{_('Prérequis *')}}</label>
                          <input type="text" wire:model="prerequis" 
                           class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm"
                            placeholder="Prerequis du client" required>
                        
                        
                          <label for="contact" class="text-muted m-2 fw-bold">{{_('Contexte ')}}</label>
                          <input type="text" wire:model="contexte" id="contact" 
                           class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm"
                            placeholder="Contexte du projet" required>
                         
                          <label for="contact" class="text-muted m-2 fw-bold">{{_('Commentaire ')}}</label>
                          <input type="text" wire:model="commentaire" id="contact" 
                           class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm" 
                           placeholder="Commentaire " required>
      ...
      {{-- modifier un projet --}}
      <div id="modal" class="form shadow w-25  {{1}}" method="POST">
