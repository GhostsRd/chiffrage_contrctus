<div>
    ...
    <div class="container   ">
      <div class="row ">
        <div class="col-lg-4">
            ...
            <h3  id="titre-prof"> Employer</h3>
            <p id="text-prof">Paramétre  / Employer</p>   
        </div>
      ...
      <div class="container   p-2 rounded-2" style="font-size: 0.75rem">
        <form wire:submit.prevent="deleteSelected">
            ...
            <th class="border-0"><input type="checkbox" ></th>
                <th class="p-2 fw-bold border-0">Nom</th>
                <th class="p-2 fw-bold border-0">Prenom</th>
                <th class="p-2 fw-bold border-0">Contact</th>
                <th class="p-2 fw-bold border-0" >
                action
                </th>
        ...
        <tbody class="p-2 border-0">
            ...
            @foreach ($employers as $dat)

                    
    
                    <tr class="border-0">
                            <td class="bg-white p-2 border-0" >
                            <input type="checkbox" wire:model="checkData" value="{{$dat->id}}" class="border-0"></td>
                        <td class="bg-white p-2 border-0" id="tdanim">{{$dat->nom}}</td>
                        <td class="bg-white  p-2 border-0" id="tdanim1">{{$dat->prenom}}</td>
  ...
  {{-- form ajout employer --}}
          <div id="modal" class="form shadow w-25  {{$form}}" method="POST">
            ...
            <div class="col-lg-12">
                <h5 class="fw-bold title-form text-center" >Ajouter un employer</h5>
                <hr>
                    <form wire:submit.prevent="create">
                    @csrf

                        <label for="projet" class="text-muted m-2 fw-bold" >{{_('nom  *')}}</label>
                        <input type="text" id="nom" wire:model='nom' class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm" placeholder="nom de l'employer" >   
                        @error('nom') <span class="error text-danger">{{ $message }} <br> </span> @enderror

                        <label for="prerequis"  class="text-muted m-2 fw-bold">{{_('prenom *')}}</label>             
                        <input type="text" wire:model="prenom"  class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm" placeholder="prenom de l'employer" >        
                        @error('prenom') <span class="error text-danger">{{ $message }} <br> </span> @enderror

                        <label for="prerequis"  class="text-muted m-2 fw-bold">{{_('Contact *')}}</label>             
                        <input type="number" wire:model="contact"  class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm" placeholder="Contact de l'employer" >        
                        @error('contact') <span class="error text-danger">{{ $message }} <br> </span> @enderror
                <hr>
        ...
        <div id="modal" class="form shadow w-25  {{$form1}}" method="POST">
          ...
          <div class="col-lg-12">
            <h5 class="fw-bold title-form text-center" >Modifier un employer</h5>
             <hr>
             
             
             <form method="POST" action="{{url('/employer/update')}}">
                
                    {{-- {{dd($data)}} --}}
                      @csrf
                            <input type="hidden" name="id" value="{{$id_employer}}">
                            ...
                            <label for="contact"  class="text-muted m-2 fw-bold">{{_('Contact *')}}</label>             
                            <input type="text" name="contact" class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm" 
