<div>
            ...
            <div class="container   p-2 rounded-2" style="font-size: 0.75rem">
              <form wire:submit.prevent="deleteSelected">
                  ...
                  <div class="row  p-2">
                      <div class="col-lg-2">
                          <h5 id="titre-prof" class="m-1 fw-bold">Liste de section</h5>
                      </div>
                  ...
                  <th class="border-0"><input type="checkbox" ></th>
                      ...
                      <th class="p-2 fw-bold border-0">Couleur du Bar</th>
              ...
              <tbody class="p-2 border-0">
                  ...
                  @foreach ($sections as $sec)
                          ...
                          <tr class="border-0">
                                  ...
                                  <input type="checkbox" wire:model="checkData" value="{{$sec->id}}" class="border-0"></td>
                              <td class="bg-white p-2 border-0" id="tdanim">{{$sec->designation}}</td>
                              <td class="bg-white  p-2 border-0" id="tdanim1">{{$sec->commentaire}}</td>
                              <td class="border-0  bg-white " style="font-size: 0.8rem;padding-left:0;height:0.7px"  >
                                <div data-aos="fade-right" data-aos-delay="800" class=" pl-0 progress-bar text-center border-0 fw-bold text-white  rounded-end-4 shadow-sm   {{$sec->couleur}}" data-toggle="progress-bar" role="progressbar" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100" style="width: 100%;height:50%; transition: width s ease 0s;"></div>
                            </td>
                             
                              <td class="bg-white  p-2 border-0" id="tdanim">
                                  <a class="text-warning" >
                                      <svg class="icon-32 " wire:click="formModifier('{{$sec->id}}','{{$sec->designation}}','{{$sec->commentaire}}')" 
                                        width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">                              
                                              <path d="M11.4925 2.78906H7.75349C4.67849 2.78906 2.75049 2.96606 2.75049 8.04806V16.3621C2.75049 19.4441 2.66949 21.6211 7.75349 21.6211H16.5775C19.6625 21.6211 21.5815 19.4441 21.5815 16.3621V12.3341" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M8.82812 10.921L16.3011 3.44799C17.2321 2.51799 18.7411 2.51799 19.6721 3.44799L20.8891 2.66499C21.8201 5.59599 21.8201 7.10599 20.8891 8.03599L13.3801 15.545C12.9731 15.952 12.4211 16.181 11.8451 16.181H8.09912L8.19312 12.401C8.20712 11.845 8.43412 11.315 8.82812 10.921Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>                                    <path d="M15.1655 2.60254L19.7315 9.16854" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>                                </svg>                            
                                  </a>
                              </td>
                          </tr>
        ...
        <div id="modal" class="form shadow w-25  {{$form}}" method="POST">
          ...
          <div class="col-lg-12">
              <h5 class="fw-bold title-form text-center" >Ajouter une séction</h5>
              <hr>
                  <form wire:submit.prevent="create">
                  @csrf

                      <label for="projet" class="text-muted m-2 fw-bold" >{{_('Désignation  *')}}</label>
                      <input type="text" id="nom" wire:model='designation'
                       class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm"
                        placeholder="Désignation du séction" required>   

                      <label for="prerequis"  class="text-muted m-2 fw-bold">{{_('Commentaire *')}}</label>             
                      <input type="text" wire:model="commentaire" 
                       class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm" 
                       placeholder="Commentaire pour la séction" required>        
              
                       <label for="prerequis"  class="text-muted m-2 fw-bold">{{_('Couleur ')}}</label>        
                       <select name="" id="" wire:model="couleur" class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm">
      ...
      <div id="modal" class="form shadow w-25  {{$form1}}" method="POST">
        ...
        <div class="col-lg-12">
          <h5 class="fw-bold title-form text-center" >Modifier une séction</h5>
           <hr>
           ...
           <form method="POST" action="{{url('/section/update')}}">
                  ...
                  {{-- {{dd($data)}} --}}
                    @csrf
                          ...
                          <label for="projet" class="text-muted m-2 fw-bold" >{{_('designation  *')}}</label>
                          <input type="text" name="designation" class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm"
                           value="{{$designation}}" required>   
      
                          <label for="prerequis"  class="text-muted m-2 fw-bold">{{_('Commentaire *')}}</label>             
                          <input type="text" name="commentaire" class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm" 
                          value="{{$commentaire}}" required>     
                          
                          <label for="contact"  class="text-muted m-2 fw-bold">{{_('Couleur *')}}</label>             
                          <select name="couleur" id="" class="form-control-plaintext p-1 ps-2 w-100  hover mt-1 rounded-5 shadow-sm">
                            <option value="">select couleur</option>
                            <option value="bg-dark">Noire</option>
                            <option value="bg-primary">Bleu</option>
                            <option value="bg-info">Bleu ciel</option>
